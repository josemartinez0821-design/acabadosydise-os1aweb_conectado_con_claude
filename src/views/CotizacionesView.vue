...
import { useCotizacionesStore, unidadServicio, tarifaServicio, ADVERTENCIA_MODO } from '../stores/cotizaciones'
...
// Aviso suave cuando a una cotización aprobada le quedan pocos días de sus `validez_dias`
// antes de vencerse (ver `estaVencida`/`diasParaVencer` en el store).
function avisoVencimiento(cot) {
  if (cot.estado !== 'aprobada') return null
  const dias = cotizStore.diasParaVencer(cot)
  if (dias === null || dias > 5 || dias < 0) return null
  return dias === 0 ? 'Esta cotización vence hoy si no la confirmas.' : `Te quedan ${dias} día${dias === 1 ? '' : 's'} para confirmar antes de que esta cotización expire.`
}
...
function confirmarPedidoServicio(cot) {
  if (!confirm(`La cotización ${cot.numero_cotizacion} por ${formatCOP(cot.total_estimado)} se confirmará como pedido. Nuestro equipo te contactará para coordinar fecha y pago. ¿Continuar?`)) return
  cotizStore.actualizarEstado(cot.id_cotizacion, 'convertida_venta')
  showToast('¡Pedido confirmado! Nuestro equipo te contactará para coordinar la fecha y el pago.', 'success')
}
...
function confirmarRechazoCliente() {
  const cot = cotizacionARechazar.value
  if (!cot) return
  cotizStore.actualizarEstado(cot.id_cotizacion, 'rechazada', notaRechazoCliente.value.trim() || 'Rechazada por el cliente.')
...
const resultadosPicker = computed(() => {
  ...
  return catalog.servicios.filter((s) => s.nombre_servicio.toLowerCase().includes(termino)).slice(0, 20)
})
...
async function enviarSolicitud() {
  ...
  const observacionesFinal = fechaDeseada.value
    ? empacarFechaDeseada(formatDate(fechaDeseada.value), observaciones.value.trim() || null)
    : observaciones.value

  cotizStore.crearCotizacion({
            ...
            <div v-for="cot in misCotizaciones" :key="cot.id_cotizacion" class="cotiz-card">
              ...
              <div v-else class="cotiz-estado-banner" :class="cotizStore.ESTADOS[cot.estado].banner">
                ...
                <div>
                  ...
                  <p v-if="avisoVencimiento(cot)" class="cotiz-estado-motivo">⏳ {{ avisoVencimiento(cot) }}</p>
                </div>
              </div>
              ...
              <div class="cotiz-card-footer">
                <template v-if="cot.estado === 'aprobada' && !cotizStore.estaVencida(cot)">
                  ...
                  <button v-else class="btn btn-primary btn-sm" @click="confirmarPedidoServicio(cot)">
