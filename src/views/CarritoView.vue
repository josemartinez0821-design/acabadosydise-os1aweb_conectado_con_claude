...
const pinturasSlider = computed(() => catalog.productosCatalogo.filter((p) => p.id_categoria === 1))
const serviciosDestacados = computed(() => catalog.servicios.filter((s) => s.destacado).slice(0, 4))
