...
const misPedidos = computed(() => {
  ...
  return MockData.ventas
    .filter((v) => v.id_usuario === auth.usuario.id_usuario)
    .map((v) => ({
      ...v,
      productos: MockData.detalle_ventas
        .filter((d) => d.id_venta === v.id_venta)
        .map((d) => ({ ...d, producto: catalog.getProductById(d.id_producto) })),
    }))
